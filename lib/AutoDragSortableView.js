...
export default class AutoDragSortableView extends Component{
    ...
    startTouch(touchIndex) {
        ...
        if (this.touchCurItem) {
            const prevIndex = this.touchCurItem.index
            const prevMoveToIndex = this.touchCurItem.moveToIndex
            // 停止上一次的缩放动画
            this.state.dataSource[prevIndex].scaleValue.stopAnimation()
            this.state.dataSource[prevIndex].scaleValue.setValue(1)
            // 恢复 zIndex
            this.touchCurItem.ref?.setNativeProps({
                style: { zIndex: defaultZIndex }
            })
            // 直接完成上一次的位置变更
            this.changePosition(prevIndex, prevMoveToIndex)
            if (this.props.onDragEnd) {
                this.props.onDragEnd(prevIndex, prevMoveToIndex)
            }
    ...
    endTouch (nativeEvent) {
        ...
        if (this.touchCurItem) {
            ...
            // 用闭包捕获当前拖拽的关键数据，防止回调触发时 touchCurItem 已被覆盖
            const capturedIndex = this.touchCurItem.index
            const capturedMoveToIndex = this.touchCurItem.moveToIndex
            const capturedRef = this.touchCurItem.ref

            // 立即清空，让新的拖拽可以正常工作
            this.touchCurItem = null

            Animated.timing(
                this.state.dataSource[capturedIndex].scaleValue,
                {
                    toValue: 1,
                    duration: this.props.scaleDuration,
                    useNativeDriver: false,
                ...
                capturedRef?.setNativeProps({
                    style: {
                        zIndex: defaultZIndex,
                    }
                })
                this.changePosition(capturedIndex, capturedMoveToIndex)
                if (this.props.onDragEnd) {
                  this.props.onDragEnd(capturedIndex, capturedMoveToIndex)
                }
            })
    ...
    changePosition(startIndex,endIndex) {
        ...
        this.setState({
            ...
            // Check and fix all elements that may be out of position
            this.state.dataSource.forEach((item, index) => {
                if (index !== startIndex && index !== endIndex) {
                    const currentX = item.position.x._value
                    const currentY = item.position.y._value
                    const targetX = parseInt(item.originLeft + 0.5)
                    const targetY = parseInt(item.originTop + 0.5)
